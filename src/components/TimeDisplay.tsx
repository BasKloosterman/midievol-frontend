...
export const TimeDisplay: FC<TimeDisplayProps> = ({curQNote, fontSize=24, color='black'}) => {
    ...
    const bars = useMemo(() => {
        return (calcMelodyLength(melodyState.melody[melodyState.curMelodyIdx]?.notes || []) / 4).toFixed(1)
    }, [melodyState.melody])
    ...
    return <div style={{display: 'flex', alignItems: 'center'}}>
        ...
        <span style={{color, marginLeft: 15}}> (total: {bars} bars of 4/4, {(calcMelodyLength(melodyState.melody[melodyState.curMelodyIdx]?.notes || []) / (configState.bpm / 60)).toFixed(2)}s)</span>
    </div>
}